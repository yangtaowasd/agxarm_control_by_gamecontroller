...
速度超过 `interaction_measured_joint_velocity_stop_limit=2.0 rad/s` 连续三个
控制周期，或单周期超过
`interaction_measured_joint_velocity_hard_limit=2.5 rad/s`，ROS 硬件 adapter
...
`interaction_measured_joint_velocity_stop_limit=2.0 rad/s` for three
consecutive cycles or exceeds the immediate
`interaction_measured_joint_velocity_hard_limit=2.5 rad/s`. Piper-L uses a
