# 文件分类索引 / File Classification Index

本索引只分类 `armbycontroller/ros/`、`armbycontroller/ik/` 和
`armbycontroller/impedance/` **之外**的仓库文件。三个排除目录仍由总学习指南
说明。mesh 数量很多，按资源目录整体归类，不逐个列出。 / This index classifies
repository files **outside** `armbycontroller/ros/`, `armbycontroller/ik/`, and
`armbycontroller/impedance/`. The excluded directories remain covered by the
main study guide. Meshes are grouped by resource tree rather than listed one by
one.
...
新控制算法先读控制与实验，再读模型与数学；需要接入键盘、手机或独立位姿节点时
