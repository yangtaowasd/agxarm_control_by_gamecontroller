...
| --- | --- |
| `CMakeLists.txt` | C++/Python 安装、节点安装、资源安装和六组 pytest 注册 / C++/Python install, node/resource install, and six pytest registrations |
...
| 文件 / File | 分类责任 / Classified responsibility |
| --- | --- |
| `config/common.yaml` | 两种机械臂共用的控制/观测周期与默认 backend / shared control/observer rates and default backend |
| `config/nero.yaml` | Nero 独立侧装裸臂、7 轴补偿/阻抗/轨迹与速度估计参数 / Nero-only side-mounted bare-arm, seven-axis compensation/impedance/trajectory, and velocity-estimation parameters |
